...
import SearchSongsComponent from "../General/search-song.component";



const MusicCatalogRightBarComponent = () => {
      ...
      const handleAiffToMp3Promise = (file, fileInfo) => {
        ...
        const handleActiveTab = (tabname) => {
          ...
          if(tabname === "Songs"){
              ...
              dispatch(saveFileManagerActive(false))
           ...
           // dispatch(saveCurrentlyEditingPlaylistName(null)) we want the playlist to persist so we canadd songs to it
            ...
            //go to lists array and set the active options to true, then set all others to false
             ...
             // dispatch(saveCurrentlyEditingPlaylistName(null)) we want playlist being edited to persist, so we can add songs to it
                 ...
                 dispatch(saveFileManagerActive(true))
...
//USE SONGS FROM FIREBASE TO PRESENT TO SCREEN
    ...
    return (
        <Box /*flex={songstab?"1.7": "1"}*/ p={2} sx={{ display: { xs: "none", sm: "block" },/*maxWidth:"35%",*/background:"#302c34",position:"relative",top:"1.1rem",borderRadius:"1rem",height:"28rem" /*,width:"25rem",*/ }}>
       ...
       <DialogModalFileCreatorComponent open={ openModal } handleClose={ handleCloseModal }  />
            ...
            <Box mt={fileManagerActive?2:0}>
             ...
             :
                ...
                <Box my={0.1} sx={{ display: "flex", justifyContent: "space-evenly",marginBottom:"0.9rem" }}>
                    { list.map( (item, key) => (
                        <Typography 
  ...
  {songstab &&
    ...
    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", width:"22rem", }}>
   ...
   {    
    <Element name="scrollContainer" style={{ overflowY: 'auto', maxHeight: '330px', scrollbarWidth: 'none', /* For Firefox */
    msOverflowStyle: 'auto' /* For IE/Edge */ }}>
      {filesFromDb &&
        ...
        {/*filesFromDb && filesFromDb.filter((item,index)=>(item.metadata && item.mediaUrl)).map((file, index) => (
                 ...
                 <KeyboardArrowLeft onClick={()=>{
                   if(sliceStart > 0 ){
                    ...
                    setSliceEnd(sliceEnd - 4)
                      ...
                      //I SAVE STARTING AND ENDING INDEX SO THAT THE DRAGGABLE WRAPPER WILL BR UP TO DATE IN WHAT IT DRAGS
                       dispatch(saveArrayStartingIndex(sliceStart - 4))
                       dispatch(saveArrayEndingIndex(sliceEnd - 4))
                   }
...
<Box sx={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
 ...
 <Element name="scrollContainer" style={{ overflowY: 'auto', maxHeight: '470px', scrollbarWidth: 'none', /* For Firefox */
msOverflowStyle: 'auto' /* For IE/Edge */ }}>
    {
   ...
   {/* playlistsFromDb && playlistsFromDb.length > 0 && 
    ...
    <div style={{display: "flex", alignItems: "flex-start", justifyContent: "center",width:"22rem",height:"5rem",/*backgroundColor:"#1C1C1E",*/borderRadius:"0.2rem",backgroundColor:"#908c8c",paddingLeft:"2.1rem",paddingRight:"2rem",cursor: "pointer"}}  onDrop={handleDrop} onDragOver={handleDragOver}>
         
          <div style={{marginTop:"2.3rem",position:"relative",top:"5px",fontSize:"12px"}}>
                      {/* File input */}
                      <input
                        type="file"
                        multiple
                        onChange={handleInputChange}
                        style={{ /*display: "none"*//*background:"pink",*/opacity:"0",userSelect:"none",height:"8rem",width:"30rem",position:"absolute",top:"-2.3rem",left:"-6rem"}}
                        id="fileInput"
                        title="" 
                      />
                      ...
                      >
                       Drag and drop files anywhere on 
                       <br/>
                       the page, or click here to browse
                      </label>
    ...
    <div style={{ marginTop: "10px",width:"100%"}}>
      ...
      {(provided) => (
        ...
        > 
             ...
             {/*I PUT OUT THE MUSIC FILE, THE INSTRUMENTAL FILE AND THE OTHERS WILL THEN BE MAPPED */}
             {/*newlyUploadedFullSong &&  newlyUploadedFullSong.mediaTemporaryUrl &&
            ...
            {(provided, snapshot) => (
              ...
              >
               ...
               <FileCreatorComponent show={false} file={newlyUploadedFullSong.file} fileFullDetails={newlyUploadedFullSong} name={newlyUploadedFullSong.name}  uploadStatus={newlyUploadedFullSong && newlyUploadedFullSong.type} handleExpandIndex={handleExpandIndex} removeFile={removeFile} indexToBeRemoved={0} fileToDelete={"mediaTemporaryUrl"}/>
              ...
              */} 
              ...
              {/*newlyUploadedFullSong &&  newlyUploadedFullSong.instrumentalTemporaryUrl && 
              ...
              {(provided, snapshot) => (
               ...
               <PlayableInstrumentalComponent show={false} file={newlyUploadedFullSong.file} fileFullDetails={newlyUploadedFullSong} name={newlyUploadedFullSong.name}  uploadStatus={newlyUploadedFullSong && newlyUploadedFullSong.type} handleExpandIndex={handleExpandIndex} removeFile={removeFile} indexToBeRemoved={0} fileToDelete={"instrumentalTemporaryUrl"}/>
              ...
              {/*newlyUploadedFullSong &&  newlyUploadedFullSong.coverArtUrl && 
               ...
               <CoverArtComponent show={false} file={newlyUploadedFullSong.file} fileFullDetails={newlyUploadedFullSong} name={newlyUploadedFullSong.name} uploadStatus={"image"} removeFile={removeFile} indexToBeRemoved={0} fileToDelete={"coverArtUrl"}/>
          ...
          {/*newlyUploadedFullSong && newlyUploadedFullSong.otherMusicFiles && newlyUploadedFullSong.otherMusicFiles.filter((item)=>(!item.mediaUrl)).map((file, index) => (
             ...
             <FileCreatorComponent show={false} file={file.file} fileFullDetails={file} name={file.name}  uploadStatus={file && file.type} handleExpandIndex={handleExpandIndex} removeFile={removeFile} indexToBeRemoved={index} fileToDelete={"otherMusicFiles"}/>
             }
...
<FileCreatorComponent show={false} file={file.file} fileFullDetails={file} name={file.name}  uploadStatus={file && file.type ==='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'||
...
*/}
                ...
                <Typography 
                    ...
                    <Box my={2} sx={{ display: "flex", justifyContent: "space-evenly",scale:"0.7",gap:"12px" }}>

                        {
                            options.map( (item, key) => (
                                <Box 
                                    px={3} py={1} 
                                    sx={{ background: "#1C1C1E", textAlign: "center", borderRadius: "7px", width: "99px",flexDirection:"column",display:"flex",justifyContent:"center",alignItems:"center" }}
                                >
                                    <Box component="img" height={item.title==="Box"?35:25} width={item.title==="Box"?35:25} src={ item.img } sx={{ justifySelf: "center" }} />
                                    <Typography mt={0.5} pb={0.5} sx={{fontSize:"10px"}}>{ item.title }</Typography>
                                </Box>
                            ) )
                        }
...
export default MusicCatalogRightBarComponent;
