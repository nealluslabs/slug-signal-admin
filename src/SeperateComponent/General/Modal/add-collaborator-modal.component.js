...
const AddCollaboratorModalComponent = ({ form, setForm,handleClose }) => {
...
console.log("WHAT IS THE SECOND HANDLE CLOSE COMPONENT-->",handleClose)
    const {selectedAudioId,
        selectedAudioState,
        currentSong,currentCollaborator} = useSelector((state)=> state.group)

    const options = [
        { title: "Contact Info", icon: <Check />, active: "contact-info" },
        { title: "Composition", icon: <Check />, active: "composition" },
        { title: "Publishing", icon: <ReportProblem sx={{ color: "#FBC756" }} />, active: "publishing" },
        { title: "Masters", icon: <Check />, active: "masters" },
        { title: "Extras", icon: <Check />, active: "extras" },
...
window.removeEventListener("resize", updateMaxChars); // Cleanup
    ...
    return (
       ...
       <Avatar
              src={currentCollaborator && currentCollaborator.profileImg?currentCollaborator.profileImg :null}
              sx={{
                width: 38,
                height: 38,
                marginLeft:"1rem",
...
export default AddCollaboratorModalComponent;
