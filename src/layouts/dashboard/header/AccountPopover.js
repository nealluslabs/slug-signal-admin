...
export default function AccountPopover() {
  ...
  return (
    <>
      ...
      >
        <Avatar  src={user && user.profilePicture?user.profilePicture:(user && user.userType==="business"? hm:redboy)} alt="photoURL" />
      </IconButton>
      ...
      >
        ...
        >
          <Grid sx={{mt: 1, ml: 1}}>
          <Avatar src={user && user.imageUrl?user.imageUrl:(user && user.userType==="athlete"?redboy:hm) } alt="photoURL" />
          </Grid>
          <Box sx={{ my: 1.5, px: 1 }}>
          <Typography variant="subtitle2" noWrap>
            {loggedInFarmer?.firstName + " " + loggedInFarmer?.lastName}
          </Typography>
          <Typography variant="body2" sx={{ color: 'text.secondary' }} noWrap>
            {loggedInFarmer && loggedInFarmer.email?loggedInFarmer.email:"default@gmail.com"}
          </Typography>
        </Box>
        </Grid>
