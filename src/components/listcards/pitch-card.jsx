...
const PitchCard = ({data,index,user,pic,collection,name,uni}) => {
  ...
  return (
    ...
    <div className={classes.row}>
      <div className={classes.text}>
        <div style={{ color: 'black',display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center" }}>
          {/*MARGIN LEFT OF 1.45 BELOW, IS EXTREMELY IMPORTANT, DO NOT CHANGE OR DELETE*/}
          <img src={pic} style={{marginLeft:"1.45rem",marginBottom:"1rem",height:"10rem",width:"13rem",borderRadius:"1.5rem"}} alt="athlete image"/>
        </div>
      ...
      <div style={{ display:"flex",flexDirection:"column",justifyContent:"flex-start",gap:"1px" ,width:"55%",paddingLeft:"8rem"}}>
          
          <span style={{fontWeight:"500",fontSize:"1.2rem",marginBottom:"-0.6rem"}}> {name}</span>
          <span  style={{fontWeight:"500",fontSize:"3rem",padding:"0rem"}}> {collection} </span>
          <span  style={{fontWeight:"500",fontSize:"1.2rem",marginTop:"-0.6rem"}}>{uni}  </span>
    ...
    >
      Endorse
            ...
            <Button
          onClick={()=>{navigate('/dashboard/crop-profile')}}
      style={{
        minHeight: '50px',
        minWidth: '180px',
