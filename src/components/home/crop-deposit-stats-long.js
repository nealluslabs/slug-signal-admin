...
const columns = [
  ...
  },*/
  {
    ...
    renderCell: (params) => {
      ...
      return <div style={{display:"flex",alignItems:"center",justifyContent:"flex-start",gap:"2.5rem",width:350,fontSize:"1rem"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"flex-start",fontSize:"1rem"}}><div>{/*THE NUMBERS SHPULD GO HERE */}</div>
  ...
  { field: 'crop', headerName: 'Crop', width: 200, renderCell: (params) => {
    return <div style={{fontSize:"1rem"}}>{params.row.cropName}</div>;
  },  },

  { field: 'earnings', headerName: 'Quantity', width: 200, renderCell: (params) => {
    return <div style={{fontSize:"1rem"}} >{params.row.earnings}  </div>;
  },  },
  { field: 'harvestDate', headerName: 'Deposit Date',  width: 220, renderCell: (params) => {
...
export default function CropDepositStatsLong({ cropDeposits }) {
  ...
  return (
    <div style={{ height:cropDeposits.length < 6?300: 650, width: '100%',paddingBottom:"0rem" }}>
